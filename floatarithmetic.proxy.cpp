...
#include <iostream>
using namespace std;
#ifndef FLOATARITHMETIC_IDL
#define FLOATARITHMETIC_IDL
...
float add(float x, float y) {
    ...
    f.appendArg("float", sizeof(float), serialize_float(x));
    ...
    if (readBuffer[0] != '0') {
        throw C150Exception("float add(float x, float y): call not recognized by the server.");
    }
...
float divide(float x, float y) {
    ...
    RPCPROXYSOCKET->read(readBuffer, 1);
    if (readBuffer[0] != '0') {
        throw C150Exception("float divide(float x, float y): call not recognized by the server.");
    }
    RPCPROXYSOCKET->read(readBuffer, sizeof(float));
    string resultStr(readBuffer, sizeof(float));
    return deserialize_float(resultStr);
...
float multiply(float x, float y) {
    ...
    RPCPROXYSOCKET->read(readBuffer, 1);
    if (readBuffer[0] != '0') {
        throw C150Exception("float multiply(float x, float y): call not recognized by the server.");
    }
    RPCPROXYSOCKET->read(readBuffer, sizeof(float));
    string resultStr(readBuffer, sizeof(float));
    return deserialize_float(resultStr);
...
float subtract(float x, float y) {
    ...
    RPCPROXYSOCKET->read(readBuffer, 1);
    if (readBuffer[0] != '0') {
        throw C150Exception("float subtract(float x, float y): call not recognized by the server.");
    }
    RPCPROXYSOCKET->read(readBuffer, sizeof(float));
    string resultStr(readBuffer, sizeof(float));
    return deserialize_float(resultStr);
